...
//Makes a copy of the top item of the parameter stack
void Sally::doDUP(Sally *Sptr){
  Token p1;

  if (Sptr->params.size() == 0){
    throw out_of_range("Parameter stack is empty.");
  }
...
//removes the top item of the parameter stack
...
//Swaps the top 2 items of the parameter stack
...
//Creates a symbol table entry with given value and name
void Sally::doSET(Sally *Sptr){
  Token p1,p2;
  
  if(Sptr->params.size() < 2){
    throw out_of_range("Need two paramenters for SET.");
  }
  ...
  string key = p1.m_text;
  int value = p2.m_value;
  
  if(Sptr->symtab.find(key) == Sptr->symtab.end()){
    SymTabEntry val = SymTabEntry(VARIABLE,value,NULL);
    Sptr->symtab.insert(pair<string,SymTabEntry>(key,val));  
  }
  else {
...
//Retries a value from symbol table and places in parameter stack
...
//Stores a given value in a symbol table entry
void Sally::doEXCLAIM(Sally *Sptr){
  ...
  if(Sptr->symtab.find(key) == Sptr->symtab.end()){
    ...
    Sptr->symtab.at(key).m_value = value;
  }
}
...
void Sally::doNOT(Sally *Sptr){
  ...
  if (Sptr->params.size() == 0){
    throw out_of_range("Need atleast one parameter for NOT.");
  }
...
void Sally::doIFTHEN(Sally *Sptr){
  ...
  if(p1.m_value != 0){
    //Sets a mark for else to validate when running
    Sptr->params.push(Token(VARIABLE, 1, ""));
    return;
  }

  //if the "IF" statement is false, code skips till it encounters else
  else {
    Token tk;
    map<string,SymTabEntry>::iterator it;
	...
	//when it encounters the "ELSE" token, it goes back to mainloop
	if (it->second.m_dothis == doELSE){
	  return;
	}
      }
    }
...
void Sally::doELSE(Sally *Sptr){
  ...
  //Runs the code under "ELSE", if the "IF" block was not skipped
  //Checks if "IF" left a mark (indicating it ran)
  if(p1.m_kind == VARIABLE && p1.m_value == 1){
    Token tk;
    map<string,SymTabEntry>::iterator it;
	...
	//skips until the code encounters "ENDIF" token
	if( it->second.m_dothis == doENDIF){
	  return;
	}
      }
    }
...
void Sally::doDO(Sally *Sptr){
  ...
  //makes a recording of all the tokens after "DO" till the "UNTIL" token
  while(tk.m_text != "UNTIL"){
    
    Sptr->recording.push_back(tk);
    tk = Sptr->nextToken();
  }

  Sptr->recording.push_back(tk);
  Sptr->params.push(Token(INTEGER,false,""));

  //once it reaches the UNTIL token it keeps it in the buffer and runs
  //the UNTIL function through here so that it loops.
  doUNTIL(Sptr);
...
void Sally::doUNTIL(Sally *Sptr){
  ...
  //if the loop condition is satisfied, runs the operations inside the loop
  if(p1.m_value == 0){

    //pushes back everything into tkBuffer
    for(list<Token>::iterator it = Sptr->recording.begin(); it != Sptr-> recording.end(); ++it){
